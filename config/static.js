...
module.exports = function (app) {
  for (appInstance of app.locals.appList.split(',')) {
    const location = path.join(__dirname, "..", app.locals.basePath, appInstance.trim(), app.locals.publicPath)
     ...
     */
    const appConfig = path.join(__dirname, "..", app.locals.basePath, appInstance.trim(), 'config', 'static.js')
  ...
  const frameworkLocation = path.join(__dirname, "..", app.locals.basePath, app.locals.publicPath)
