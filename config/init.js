...
module.exports = function (app) {
  if (app.locals.httpsOn) {
    const tlsKeyPath = path.join(
      __dirname,
      "..",
      app.locals.tlsPath,
      app.locals.tlsServerKey,
    );
    const tlsCertPath = path.join(
      __dirname,
      "..",
      app.locals.tlsPath,
      app.locals.tlsServerCert,
    );
    ...
    try {
      ...
      app.locals.debug &&
        console.debug("✅  TLS credentials loaded successfully");
    } catch (err) {
      console.error(
        `⚠️  Could not read TLS key/cert files at ${tlsKeyPath} and ${tlsCertPath}. HTTPS will be disabled.`,
      );
  ...
  app.use(
    helmet({
      contentSecurityPolicy: app.locals.contentSecurityPolicy,
    }),
  );
  ...
  if (app.locals.rateLimitFifteenMinuteWindow > 0) {
    ...
    app.locals.debug &&
      console.debug(
        `✅  Rate limiting is enabled: ${app.locals.rateLimitFifteenMinuteWindow} requests per 15 minutes`,
      );
  } else {
